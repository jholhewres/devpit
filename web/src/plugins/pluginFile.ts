...
/*
 * What every Capability that keeps files needs, before any of them loads.
 *
 * The menus and the mount table import this; the editors stay behind
 * `import()`, so nothing here may reach one of them.
 *
 * Lifted out of `excalidraw/drawings.ts` when the second Capability arrived.
 * The name rules are the backend's, spelled here so a refusal is said before
 * the round trip — a second copy of them would be a second answer.
 */

/** How long a file waits after the last change before it is written. */
export const SAVE_AFTER_MS = 800

/** Past any name a person gives a file, and inside the 255 bytes every
    filesystem devpit runs on allows. Mirrors `LONGEST_DATA_NAME`. */
...
function refusedInAName(codePoint: number): boolean {
  return (
    codePoint <= 0x1f ||
    codePoint === 0x7f || // C0 controls and DEL, as `char::is_control`
...
/** What a Capability needs to know about its own files, in one place. */
export interface FileKind {
  /** The manifest id in `catalogue()`. */
  readonly pluginId: string
  /** With the dot. A file this Capability keeps ends in it. */
  readonly extension: string
  /** The word for one of them on screen, lower case: "drawing", "note". It
   *  is also what a tab of one is called, so a tab already open under it
   *  keeps its identity when a Capability is renamed. */
  readonly noun: string
  /** What a new one starts as. */
  readonly empty: string
}

export const stemOf = (kind: FileKind, name: string): string =>
  name.endsWith(kind.extension) ? name.slice(0, -kind.extension.length) : name

/** One tab per file: the id is the name, so an open file is focused rather
    than opened twice. */
export const fileTab = (kind: FileKind, name: string): Partial<Tab> => ({
  id: `${kind.noun}:${name}`,
  path: name,
  title: stemOf(kind, name),
})

/** The file a typed name becomes, or why it cannot become one. */
export function fileName(kind: FileKind, typed: string): Named {
  const stem = stemOf(kind, typed.trim())
  if (stem === '') return { problem: `Give the ${kind.noun} a name.` }
